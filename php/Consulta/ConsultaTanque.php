<div class="container">
  <div class="row">
    <div class="col-lg-5 col-lg-offset-1">
      <div class="input-group">
        <button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#suministro">
            <span class="glyphicon glyphicon-scale"></span> Nuevo suministro de oxigeno
        </button>
      </div>
    </div>
    <div class="container">
      <div class="col-lg-3">
        <div class="input-group">
            <span class="input-group-btn">
          <input type="text" class="form-control" placeholder="Buscar">
          <button class="btn btn-info btn-lg" type="button" >
            <span class="glyphicon glyphicon-search"></span> Consulta
          </button>
          <button class="btn btn-warning btn-lg" type="button">
              <span class="glyphicon glyphicon-minus-sign"></span> Limpiar
          </button> 
          </span>                             
        </div>      
      </div>  
    </div>
  </div>
  <br>
  <div class="col-xs-12">
    <div class="row">
      <div class="table-responsive">
        <table class="table table-striped">
          <thead>
            <tr class="table-bordered">
              <td >ID           </td>
              <td >Suministro   </td>
              <td >Precio       </td>
              <td >Acción       </td>
            </tr>
          </thead>
          <tbody>
              <tr>
                  <td >Mark</td>
                  <td >Otto</td>
                  <td >@mdo</td>
                  <td >Otto</td>
              </tr>
          </tbody>
        </table>
      </div>
    </div>
  </div>
</div>

  <!-- Navegacion del usuario administrador del sistema PRAXAIR-->
    <?php include "Modales.php"; ?>       
  <!-- Fin-->






